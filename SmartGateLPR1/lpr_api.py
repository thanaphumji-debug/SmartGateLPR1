...
#
# ⛔ กัน torch ไม่ให้ถูกลากซ้ำ / กัน modelscope ทำให้โหลดช้าลงโดยไม่จำเป็น
#
# paddleocr ลาก modelscope เข้ามาเป็น "แหล่งดาวน์โหลดโมเดลสำรอง" ผ่านลูกโซ่นี้:
#     paddleocr/__init__.py
#       └─ paddlex/inference/utils/official_models.py:33  →  import modelscope
#            └─ modelscope/utils/torch_utils.py:14        →  import torch (ซ้ำ)
# ไม่ได้ทำให้พัง (torch โหลดแล้วจาก YOLO ด้านบน) แต่โหลดช้าลงและดาวน์โหลด
# โมเดลผ่าน huggingface อยู่แล้วเป็นค่าเริ่มต้น ไม่จำเป็นต้องมี modelscope เลย
# ปิดได้ด้วย LPR_ALLOW_MODELSCOPE=1 ถ้าจำเป็นต้องโหลดโมเดลผ่าน modelscope จริง ๆ
if os.environ.get("LPR_ALLOW_MODELSCOPE", "0") != "1" and "modelscope" not in sys.modules:
    import types as _types

    _stub = _types.ModuleType("modelscope")
    _stub.__doc__ = "โมดูลปลอมของ SmartGateLPR — กัน paddleocr โหลด modelscope โดยไม่จำเป็น"

    def _modelscope_disabled(*args, **kwargs):
        raise RuntimeError(
            "modelscope ถูกปิดไว้ (ไม่จำเป็น เพราะใช้ huggingface เป็นแหล่งดาวน์โหลดหลักอยู่แล้ว) "
            "ถ้าจำเป็นต้องโหลดโมเดลผ่าน modelscope จริง ๆ ให้ตั้ง LPR_ALLOW_MODELSCOPE=1 แล้วรันใหม่"
        )
...
print("⏳ กำลังโหลด PaddleOCR...", flush=True)
_t_paddle = time.time()
from paddleocr import PaddleOCR
print(f"   ✔ paddleocr โหลดเสร็จ ({time.time() - _t_paddle:.1f}s)", flush=True)

# ---------- PaddleOCR รันบน CPU เสมอ ----------
#
# ⛔ ไม่ auto-detect / ไม่รองรับ GPU สำหรับ PaddleOCR ในโปรเซสนี้โดยเจตนา
#
# เคยลองแยก PaddleOCR ไปรันอีกโปรเซสหนึ่งเพื่อให้ใช้ paddlepaddle-gpu ได้
# (กันชนกับ torch/YOLO ที่อยู่โปรเซสนี้) แต่เจอปัญหาต่อเนื่องหลายอย่างระหว่าง
# ทาง (ติดตั้ง paddlepaddle-gpu พัง, ตรวจจับป้ายได้แต่อ่านตัวอักษรไม่ออกเลย)
# ซึ่งไล่ทีละปัญหาเปลืองเวลามาก จึงย้อนกลับมาเป็นไฟล์เดียวแบบเดิม — รันบน
# CPU เท่านั้น (paddlepaddle รุ่น CPU ธรรมดา ไม่ใช่ paddlepaddle-gpu)
#
# ถ้าติดตั้ง paddlepaddle-gpu ไว้ในโปรเซสเดียวกับ torch อาจพังตอน import ด้วย
#     ImportError: generic_type: type "_gpuDeviceProperties" is already registered!
# (torch กับ paddle รุ่น GPU ผูก struct ของ CUDA เข้ากับ pybind11 ด้วยชื่อ
#  ชนิดข้อมูลเดียวกัน) — อย่าลง paddlepaddle-gpu ถ้าจะรันไฟล์นี้ไฟล์เดียว
PADDLE_DEVICE = "cpu"
print("⚠️  PaddleOCR: รันบน CPU (ตั้งใจ — ไม่รองรับ GPU ในโหมดโปรเซสเดียว)")

print(f"⏳ กำลังโหลดโมเดล PaddleOCR: อ่านตัวอักษร={PADDLE_REC_MODEL} | "
      f"ตรวจจับข้อความ={PADDLE_DET_MODEL}...", flush=True)
print("   (ครั้งแรกต้องดาวน์โหลดโมเดลก่อน อาจใช้เวลาสักครู่)", flush=True)
...
#   enable_new_ir=False    -> ไม่ใช้ executor รุ่นใหม่ (PIR)
#
# ⛔ ห้ามเปิด oneDNN (mkldnn) บน CPU กับ paddlepaddle รุ่นที่ใช้อยู่ — จะพังด้วย
#      (Unimplemented) ConvertPirAttribute2RuntimeAttribute not support
#      [pir::ArrayAttribute<pir::DoubleAttribute>]
#      (at ...new_executor/instruction/onednn/onednn_instruction.cc:118)
# enable_new_ir=False ไม่ช่วยแก้บั๊กนี้ (new IR กับ new executor เป็นคนละสวิตช์
# ดู paddlex/inference/models/runners/paddle_static/runner.py:493-494 —
# executor รุ่นใหม่ถูกเรียกตายตัว ปิดไม่ได้) แต่ยังตั้งไว้เผื่ออนาคต
#
...
_ocr_kwargs = dict(
    ...
    use_textline_orientation=PADDLE_TEXTLINE_ORI,   # ปิด — เราดัดป้ายเองแล้วด้วย deskew_plate()
...
print("✅ AI พร้อมทำงานแล้ว! สแตนด์บายรอรับรูปภาพที่ Port 5000", flush=True)
...
def read_plate_paddle(plate_img, return_raw=False):
    ...
    เช่น ไม่เจอบรรทัดจังหวัดเลย VS เจอแต่ดัดไม่ตรง)
...
def predict():
    ...
    try:
        ...
        # --- 3. อ่านตัวอักษรบนป้าย (PaddleOCR) ---
        if SAVE_DEBUG_PLATE:
...
if __name__ == "__main__":
    # threaded=True ให้ /detect กับ /predict รับพร้อมกันได้ (model_lock กันเรียกโมเดลซ้อน)
